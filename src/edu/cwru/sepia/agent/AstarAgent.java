...
public class AstarAgent extends Agent {
    ...
    {
        public int x, y;
        public final MapLocation cameFrom;
        public int cost;
        ...
        public int heuristic(MapLocation goal) {
            int xDiff = Math.abs(goal.x - x);
            int yDiff = Math.abs(goal.y - y);
            int D = Math.max(xDiff, yDiff);  // Chebyshev distance
            return cost + D;
        }
     ...
     *
     * @param state State view to be factored into decision
     * @param history History view to be factored into decision
     * @param currentPath Path currently being taken
     * @return Decision whether the current path should be replanned to account for unexpected blockage
     */
    private boolean shouldReplanPath(State.StateView state, History.HistoryView history, Stack<MapLocation> currentPath)
    {
        if (enemyFootmanID == -1 || currentPath.isEmpty())
            return false;
        Unit.UnitView unit = state.getUnit(enemyFootmanID);
        enemyLoc = new MapLocation(unit.getXPosition(), unit.getYPosition(), null, 0);
        return enemyLoc.equals(currentPath.peek());
    }
    ...
    private Stack<MapLocation> AstarSearch(MapLocation start, MapLocation goal, int xExtent, int yExtent, MapLocation enemyFootmanLoc, Set<MapLocation> resourceLocations)
    {
        ...
        Stack<MapLocation> open = new Stack<>();
        Stack<MapLocation> closed = new Stack<>();
        open.push(start);

        // Loop until open list is empty
        while (!open.isEmpty())
        {
            // find heuristically optimum location on open list
            MapLocation current = open.peek();
            int opt = current.heuristic(goal);
            for (MapLocation loc : open) {
                int test = loc.heuristic(goal);
                if (test < opt) {
                    current = loc;
                    opt = test;
                }
            ...
            // generate successors of current location
            ArrayList<MapLocation> successors = new ArrayList<>(8);
            int x1 = Math.max(0, current.x-1);
            int x2 = Math.min(current.x+1, xExtent-1);
            for (int x = x1; x <= x2; x++) {
                int y1 = Math.max(0, current.y-1);
                int y2 = Math.min(current.y+1, yExtent-1);
                for (int y = y1; y <= y2; y++) {
                    if (x==current.x && y==current.y)
                        continue;
                    MapLocation suc = new MapLocation(x, y, current, current.cost+1);
                    boolean blocked = false;
                    for (MapLocation resLoc : resourceLocations)
                        if (suc.equals(resLoc)) {
                            blocked = true;
                            break;
                        }
                    blocked = blocked || (enemyFootmanLoc != null && suc.equals(enemyFootmanLoc));
                    if (!blocked)
                        successors.add(suc);
                }
            }

            // add appropriate successors to open list
            for (MapLocation suc : successors) {
                if (suc.equals(goal)) {
                    Stack<MapLocation> path = new Stack<>();
                    MapLocation pathLoc = suc.cameFrom;
                    while (pathLoc.cameFrom != null) {
                        path.push(pathLoc);
                        pathLoc = pathLoc.cameFrom;
                    }
                    return path;
                }
                if (ignoreSuccessor(open, suc, goal) || ignoreSuccessor(closed, suc, goal))
                    continue;
                open.push(suc);
            }
        ...
        // return null stack if no available path
    ...
    /**
     * This method determines whether the successor's location is already in the specified list, and should be ignored.
     * @param list A list of locations
     * @param suc A candidate location to be added to the path
     * @param goal MapLocation of the townhall
     * @return true if suc should be ignored; false if it should be allowed to be eventually added to the open list
     */
    private boolean ignoreSuccessor(Stack<MapLocation> list, MapLocation suc, MapLocation goal) {
        for (MapLocation loc : list)
            if (loc.equals(suc)) {
                int floc = loc.heuristic(goal);
                int fsuc = suc.heuristic(goal);
                if (floc < fsuc)
                    return true;
            }
        return false;
